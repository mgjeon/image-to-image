...
# Load config ==================================================================
    with open("configs/default_fabric.yaml") as file:
...
# Fabric =======================================================================
    L.seed_everything(fab['seed'])
    torch.set_float32_matmul_precision(fab['float32_matmul_precision'])
    
    fabric = L.Fabric(accelerator=fab["accelerator"], 
                      devices=fab["devices"],
                      precision=fab["precision"],
                      strategy=fab["strategy"])
    fabric.launch()
...
# Setup Training ===============================================================
    ...
    if params['resume_from'] is not None:
        checkpoint = fabric.load(params['resume_from'])
        ...
        checkpoint = fabric.load(output_model_dir/"latest_checkpoint.pth")
        start_epoch = checkpoint['epoch'] + 1
        start_iteration = checkpoint['iteration']
        if start_epoch > params['num_epochs']:
            raise ValueError(f"Resuming epoch {start_epoch} is greater than num_epochs {params['num_epochs']}")
...
# Save latest checkpoint ======================================================
        if fabric.is_global_zero:
            ...
            fabric.save(output_model_dir/"latest_checkpoint.pth", state)
...
# Save model ===================================================================
        if epoch % params['save_state_per_epoch'] == 0 or epoch == 1:
            if fabric.is_global_zero:
                ...
                # fabric.save(output_model_dir/f"{epoch}_{iteration}_D.pth", net_D.state_dict())
